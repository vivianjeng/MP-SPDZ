...
def mode(data: list[sint]):
    ...
    @for_range(n)
    def _(i):
        freqs[i] = sum(if_else(x == data[i], 1, 0) for x in data)

    # set the frequencies of the magic number to zero
    @for_range(n)
    def _(i):
        freqs[i] = sum(if_else(x == MAGIC_NUMBER, 0, freqs[i]) for x in data)
